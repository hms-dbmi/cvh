...
export default function InfiniteScrollList<T extends { uuid: string }>({
    ...
    useEffect(() => {
        ...
        if (!hasMoreItems || isLoading || isError || !loadMoreItems || !parentRef.current) return;


        // Load more items when the last item in the virtualizer is visible
        const lastItem = virtualizer.getVirtualItems().slice(-1)[0];
        const scrollLocation = parentRef.current.scrollTop + parentRef.current.scrollHeight || 0;
        
        if (lastItem.start < scrollLocation) {
            loadMoreItems();
        }

    }, [virtualizer, hasMoreItems, isLoading, isError, loadMoreItems, items.length]);
...
function VirtualRow({height, start, children}: PropsWithChildren<{height: number, start: number}>) {
    return (
        <div
            style={{
                position: 'absolute',
