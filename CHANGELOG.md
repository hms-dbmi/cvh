...
- Google Analytics 4 (gtag.js) is now loaded on dev and prod for pageview tracking. Implicit tracking only — no manual event capture; TanStack Router navigations are picked up automatically via GA4's Enhanced Measurement (History API listener). Sets the standard `_ga` / `_ga_*` cookies. Dev traffic lands in a separate GA property from prod, and local `npm run dev` doesn't fire GA at all (the `inject-google-analytics` Vite plugin only injects the snippet when `VITE_GA_MEASUREMENT_ID` is set at build time).
